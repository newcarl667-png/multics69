	...
	while (1) {
		...
		if (readlen==-1) {
			if (errno==EINTR) continue;
				...
				debugf(0," pipe(%d): read failed (errno=%d)", fd, errno);
				return -1;
			}
		}
		break;
	...
	while (1) {
		...
		if (writelen==-1) {
			if (errno==EINTR) continue;
				...
				debugf(0," pipe(%d): write failed (errno=%d)", fd, errno);
				return -1;
			}
		}
		break;
...
int pipe_recv( int fd, uchar *buf )
{
	...
	if (rlen!=4) {
		debugf(0," pipe(%d): header recv error (rlen=%d)\n",fd,rlen);
		pipe_purge(fd); // wrong data --> purge
		return 0;
	}
	if (rbuf[0]!=0xFF) {
		debugf(0," pipe(%d): recv error, wrong id %02X\n", fd,rbuf[0]);
		pipe_purge(fd); // wrong data --> purge
		return 0;
	}
	...
	if ( rlen!= len+4 ) {
		debugf(0," pipe(%d): recv error (rlen=%d)\n",fd,rlen);
		pipe_purge(fd); // wrong data --> purge
		return 0;
	}
	//Checksum
	...
	if (sum!=rbuf[1]) {
		debugf(0," pipe(%d): recv error, wrong checksum\n",fd);
		pipe_purge(fd); // wrong data --> purge
		return 0;
	}
	memcpy(buf, rbuf+4, len);
	//debugf(0," pipe(%d): recv data\n"); debughex(rbuf, rlen);
	return len;
}	
