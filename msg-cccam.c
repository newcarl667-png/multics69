...
int cc_msg_recv(int handle,struct cc_crypt_block *recvblock, uint8 *buf, int timeout)
{
	...
	if (len<=0) {
		debugf( getdbgflag(DBG_CCCAM,0,0), " CCcam: recv error %d(%d)\n",len, errno);
		return len;
	}

	if (len != 4) { // invalid header length read
		debugf( getdbgflag(DBG_CCCAM,0,0), " CCcam: invalid header length\n");
	...
	if (((netbuf[2] << 8) | netbuf[3]) != 0) {  // check if any data is expected in msg
		if (((netbuf[2] << 8) | netbuf[3]) > CC_MAXMSGSIZE - 2) {
			debugf( getdbgflag(DBG_CCCAM,0,0), " CCcam: message too big\n");
			return -1;
		}
		...
		if (len != ((netbuf[2] << 8) | netbuf[3])) {
			debugf( getdbgflag(DBG_CCCAM,0,0), " CCcam: invalid message length read %d(%d)\n",len,errno);
			return -1;
		}
